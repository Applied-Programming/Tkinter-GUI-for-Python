...
class App:
    def __init__(self, master):
        ...
        #Course Applied For
        Label(master, text="Course Applied for:", wraplength=60).grid(row=4) #, column=3)
        self.L1 = Listbox(master, width = 25, exportselection=0, height = 4)
        for item in ["Quality Management (Adv.)", "Financial Management (Adv.)", "Project Management (Adv.)", "Project Management (Int.)"]:
        ...
        self.w=Button(self.f2, text ="Prerequisites", height =1, width=10, command=self.Chk_Prereq, default=ACTIVE).pack()
        self.w1=Button(self.f2, text ="Clear", height =1, width=10, command=self.Clear).pack()
        self.w2=Button(self.f2, text ="Cancel", height=1, width=10, command=self.Close).pack()
        ...
        #Checkbox
        self.var=IntVar()
        self.c=Checkbutton(master, text="Part-Time Course", variable= self.var, offvalue=0, onvalue=1)
        self.c.grid(row=7)
    ...
    def Eval(self):
        ...
        #Check for Age
        if self.age < 21:
            tkMessageBox.showwarning("Invalid Age", "You are not eligible")
            return
        ...
        self.varl1 = self.L1.curselection()
